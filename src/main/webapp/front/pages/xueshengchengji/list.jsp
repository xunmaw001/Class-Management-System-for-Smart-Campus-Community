				...
				<form class="layui-form filter" :style='{"padding":"20px 10px","alignItems":"center","flexWrap":"wrap","background":"#f5f5f5","display":"flex","width":"100%","justifyContent":"center","height":"auto"}'>
					...
					<div :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}' class="item-list">
						...
						<select name="xueshengxuehao" id="xueshengxuehao" lay-filter="xueshengxuehao">
							...
							<option v-for="(item,index) in xueshengxuehaoOptions" v-bind:key="index" :value="item">{{item}}</option>
						</select>
					</div>
					<div :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}' class="item-list">
						<div class="lable" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px"}'>等级</div>
						<select name="dengji" id="dengji" lay-filter="dengji">
							<option value="">请选择</option>
							<option v-for="(item,index) in dengjiOptions" v-bind:key="index" :value="item">{{item}}</option>
						</select>
					</div>
			...
			layui.use(['form', 'layer', 'element', 'carousel', 'laypage', 'http', 'jquery','laydate', 'slider'], function() {
				...
				http.request('config/list', 'get', {
        			...
        			http.request(`option/xuesheng/xueshengxuehao`,'get',{},(res)=>{
                			vue.xueshengxuehaoOptions = res.data
							vue.$nextTick(() => {form.render()})
        			});
			        vue.dengjiOptions = '优秀,良好,及格,不及格'.split(',')
					vue.$nextTick(() => {form.render()})
				...
				function pageList() {
			        ...
			        if (jquery('#dengji').val()) {
			          param['dengji'] = jquery('#dengji').val() ? jquery('#dengji').val() : '';
			        }
			        param['sort'] = 'dengjishijian';
			        param['order'] = 'desc';
				...
				if (jquery('#dengji').val()) {
					...
					// 获取列表数据
					http.request('xueshengchengji/list', 'get', param, function(res) {
